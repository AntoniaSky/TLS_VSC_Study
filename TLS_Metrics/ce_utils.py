...
def read_las(file_path):
    ...
    with laspy.open(file_path) as lasfile:
        ...
        y = lasdata.y
        z = lasdata.z
        classification = lasdata.classification
        #todo classification --> filter ground points by classification
        points = np.vstack([x, y, z]).transpose()
    print("Read point cloud with shape: {}".format(points.shape))
    return points, classification


def filter_pts(points, classification):#, above_ground=1):
    """Filtering ground point clouds
    Args:
        points:             np.array(n, 3), point cloud using x, y, and z
        classification:     np.array(n,), classification of each point
        above_ground:       float,          the width of ground, a buffer parameter

    Return:
        points:             np.array(m, 3), non-ground points
    """
    not_ground = classification != 1 # Assuming 1 is the ground class
    print("Filterng points finished")
    return points[not_ground] #only 


# ---------------------------Resampling strategy------------------------------
def adaptive_sample_pts(points, layer_height=1, return_sample_info=False, is_figure=True, square="alpenplatz",
    ...
    def cal_avg_layer_distance(points_pcd):
        print("Calculating the average point distance along height")
    ...
    if is_figure:
        ...
        plt.tight_layout()
        plt.savefig(fig_path+f'_before_{square}.jpg', dpi=600)
        plt.clf()
        plt.close()
    ...
    for ratio in np.arange(2.0, 5.3, 0.3):
        ...
        print("Voxel size: {}, MK-test result: {}, p-value: {}".format(voxel_size, mk_result.h, mk_result.p))
        print("MK-test result: {}".format(mk_result))
        if is_figure:
            if not os.path.exists(os.path.dirname(fig_path)):
                os.makedirs(os.path.dirname(fig_path))
            plt.figure(figsize=(4/2.54, 6/2.54))
            plt.plot(avg_layer_dis['diss'].values, avg_layer_dis['layers'].values)
            plt.tight_layout()
            plt.savefig(fig_path+f'_{ratio}_{square}.jpg', dpi=600)
            plt.clf()
            plt.close()
        if not mk_result.h:
            print("MK-test result is finally not significant, stop resampling.")
            break
    print("Adaptive sampling finished as eather the MK-test result is not significant or the ratio reaches 4.0.")
    if is_figure:
        plt.figure(figsize=(4/2.54, 6/2.54))
        plt.plot(avg_layer_dis['diss'].values, avg_layer_dis['layers'].values)
        plt.tight_layout()
        plt.savefig(fig_path+f'_after_{square}.jpg', dpi=600)
        plt.clf()
        plt.close()
